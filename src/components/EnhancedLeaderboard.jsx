// src/components/EnhancedLeaderboard.jsx
import { useState, useEffect } from 'react';
import { supabase } from '../lib/supabase';

export default function EnhancedLeaderboard({ challengeId }) {
    ...
    const fetchLeaderboard = async () => {
      try {
        ...
        data?.forEach(log => {
          ...
          if (!userDailyData[userId][log.log_date]) {
            userDailyData[userId][log.log_date] = 0;
          }
          
          userDailyData[userId][log.log_date] += log.exercise_count;
        });
        
        setDailyBreakdown(userDailyData);
            ...
            <div key={user.userId}>
              ...
              >
                ...
                <div className="count-column">
                  ...
                  <span className="expand-icon">{expandedUser === user.userId ? '▲' : '▼'}</span>
                </div>
              </div>
              
              {expandedUser === user.userId && timeFrame !== 'today' && (
                  ...
                  <div className="daily-data">
                    {last7Days.map(day => (
                      <div key={day.date} className="day-column">
                        ...
                        <div className={`day-count ${dailyBreakdown[user.userId]?.[day.date] ? 'active' : ''}`}>
                          {dailyBreakdown[user.userId]?.[day.date] || 0}
                        </div>
                      </div>
                    ))}
                  </div>
